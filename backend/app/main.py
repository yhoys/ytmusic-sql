...
@app.get("/export")
def export_library():
    """Exports the full library to an Excel file."""
    conn = get_db()
    cursor = conn.cursor()
    ...
    cursor.execute("""
        SELECT
            ...
            s.duration_sec, s.language, s.yt_video_id,
    ...
    ws2.append(["Playlist", "Total Songs", "Autogenerated"])
    for cell in ws2[1]:
        cell.font = Font(bold=True, color="FFFFFF")
        cell.fill = PatternFill("solid", fgColor="CC0000")
        cell.alignment = Alignment(horizontal="center")
    ...
    cursor.execute("""
        SELECT p.name, COUNT(ps.song_id) AS total, p.is_autogenerated
        FROM playlists p
        LEFT JOIN playlist_songs ps ON p.id = ps.playlist_id
        GROUP BY p.id, p.name, p.is_autogenerated
        ORDER BY p.id;
    ...
    for row in cursor.fetchall():
        ws2.append(
            [row["name"], row["total"], "Yes" if row["is_autogenerated"] else "No"]
        )
